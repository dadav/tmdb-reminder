...
`just up` and `just up-local` start PostgreSQL first and run
`scripts/sync-postgres-password.sh` over its trusted local socket. This keeps the
persisted role aligned when `POSTGRES_PASSWORD` changes without deleting the
volume.
...
- **Reconciliation:** release events store TMDB's original `source_date` and the effective `scheduled_date`. A changed source date or configured delay supersedes the current event and creates a new revision (cancelling that event's unsent deliveries); if any prior revision was already delivered, the new delivery is flagged `is_revised`. A disappeared movie date withdraws the current event. Delayed TV episodes stay current when TMDB advances, so multiple episode events may overlap. A terminal TV delivery (`sent`, `expired`, or `cancelled`) remains current through its effective release day; the hourly delivery run retires it after `expiry_at`, revealing the next current episode.
- **Movie availability:** `select_movie_release` (`tmdb/mapping.py`) derives, per `TMDB_REGION`, an `available_since` (earliest type 4/5/6 digital/physical/TV date at or before today) and a `next_digital_date` (earliest future type-4 date, only when not already available). Availability outranks a later digital release: an available movie carries no reminder, withdraws any current digital event and cancels its unsent deliveries, stores `available_since` on the title, and becomes `completed` under the normal revision-watch calc. A corrected availability date updates the stored date and watch deadline. If availability disappears the date clears and the movie reopens to `active` with either its future digital reminder or an unknown state. Stopped/dormant metadata refreshes update `available_since` (and `availability_source`) without touching status or reminders.
- **Watch-provider fallback:** when release selection returns both `available_since=None` and `next_digital_date=None`, `TrackingService._fetch` consults `TmdbAdapter.movie_watch_providers`; `movie_available_from_providers` (`tmdb/mapping.py`) marks the movie available when the configured region has a non-empty `flatrate`/`free`/`ads`/`rent`/`buy` list. This is an **undated** availability: it completes the movie under revision-watch from the observation date, withdraws any current digital reminder, creates no notification, and stores no date. `availability_source` (enum `release_date`/`watch_provider`, nullable, CHECK-coupled to `available_since`) records how availability was set; null means unknown. Provider availability is **sticky** - a later provider disappearance or future digital date never reopens tracking (dated availability is not sticky and still reopens); a later past release date upgrades it to a dated availability. Provider fetch failure fails the whole fetch and preserves prior state. `TitleView`/`SearchResultItem` expose `is_available` (movies only; dated or undated) plus `available_since`, suppress `next_release` for available movies, and drive the completed-movie Remove button. Watch-provider data requires JustWatch attribution in the footer alongside the TMDB attribution.
- **Movie lifecycle:** a movie also becomes `completed` after its digital event is delivered or expires; either way it stays under revision-watch (`revision_watch_until`, default release + 30 days) and reopens to `active` if a new date appears. TV stays `active` until stopped. Stop is a soft status change (`stopped`) that cancels unsent deliveries and preserves history; there is no purge in v1.
...
- **Availability delay:** `AVAILABILITY_DELAY_DAYS` is a global 0-30 calendar-day delay (default 0) applied to dated movie types 4/5/6 and TV episode air dates. Shifted effective dates drive UI labels, deliveries, expiry, availability, and movie lifecycle. Provider fallback remains immediate and never overrides a dated schedule. Normal refresh rebases actionable events; stopped and dormant history is preserved.
