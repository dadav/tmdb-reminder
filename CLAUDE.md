...
- **Reconciliation:** release events store TMDB's original `source_date` and the effective `scheduled_date`. A changed source date or configured delay supersedes the current event and creates a new revision (cancelling that event's unsent deliveries); if any prior revision was already delivered, the new delivery is flagged `is_revised`. A disappeared movie date withdraws the current event. Delayed TV episodes stay current when TMDB advances, so multiple episode events may overlap until each delivery is sent or expired.
